...
class BaiduMQuanBuSpider(scrapy.Spider):
    ...
    def __init__(self, *args, **kwargs):
        ...
        exesql = settings['GET_HUICONG_YUQING_KEYWORD_SQL'].format('baidu_m_zixun'.upper(), settings['SELECT_STEP'])
    ...
    def start_requests(self):
        ...
        for index, keyword, page_max in self.start_urls:
            ...
            for pn in range(0, pn_max, 10):
                ...
                item['dropdown'] = ' '
                item['related'] = ' '
                cur_url = 'http://m.baidu.com/sf/vsearch?'
                ky = keyword.encode('gb18030')
                url = cur_url + 'word=%s&pd=realtime&tn=vsearch&pn=%d&sa=vs_tab&mod=5&p_type=1&data_type=json&atn=index' % (ky, pn)
    ...
    def parse(self, response):
        ...
        contents = contents['data']['list']
        for cur_rank, content in enumerate(contents):
            page_title_url = content['titleurl']
            item['media_name'] = content['subsitename_new']
            item['title'] = content['title'].replace('<em>', '').replace('</em>', '')
